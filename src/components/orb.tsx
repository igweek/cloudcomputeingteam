...
function hslToRgb(h: number, s: number, l: number) {
  ...
  const p = 2 * l - q;
  return new Vec3(hueToRgb(p, q, h + 1 / 3), hueToRgb(p, q, h), hueToRgb(p, q, h - 1 / 3));
}
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        backgroundColor: { value: colorToVec3(backgroundColor) },
      },
    });
    ...
    const update = (time: number) => {
      animationFrame = 0;
      const delta = lastTime ? Math.min((time - lastTime) * 0.001, 0.1) : 0;
      lastTime = time;
      ...
      if (!reduceMotion && isIntersecting && !document.hidden) {
